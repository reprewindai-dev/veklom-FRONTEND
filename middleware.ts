 ...
 *
 * This middleware performs only a navigation/session-presence check. LockerPhycer
 * owns authentication/session state; CAPPO remains the hard consequence-authority
 * boundary. Public Capability OS and proof navigation are intentionally not gated.
 *
 ...
 *
 * Both are presence signals only. The canonical backend still validates the actual
 * token, server-side session, workspace binding and account status.
 */
