...
## Context
With the shift to the Canonical Architecture where "Connection is the Product," there needs to be strict separation of concerns across the Veklom repositories to prevent circular authority and fragmented sources of truth. Previous iterations allowed overlapping responsibilities (e.g., CAPPO evaluating CAPPO, transient gateway state acting as truth, PGL using mocked receipts).
...
### 2. `reprewindai-dev/cappo-backend`
...
### 4. `reprewindai-dev/veklom-FRONTEND`
**Role:** Presentation Layer
- **Owns:** Presentation and operator actions only.
- **Constraints:** Cannot construct hardcoded statuses or topologies. Must consume the canonical backend capability/status manifest. Legacy `capi.veklom.com` hostnames and `CAPI_*` environment names are deployment compatibility labels for the CAPPO governed runtime, not a separate implementation repo.
