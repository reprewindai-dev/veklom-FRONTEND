...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#0A0A0A] text-white overflow-x-hidden selection:bg-blue-500/30">
      ...
      {/* Value Scroll Section 3: Lifecycle Governance */}
      <section className="py-24 relative overflow-hidden bg-[#0A0A0A]">
        <div className="max-w-7xl mx-auto px-6">
          ...
          >
            ...
            {/* Architecture Diagram — inline SVG fallback */}
            <motion.div
              variants={fadeUpVariants}
            ...
            >
              <div className="p-8 grid grid-cols-3 gap-6 text-center">
                {[
                  { label: "SEKED", sub: "Policy-checked execution", color: "blue" },
                  { label: "PGL", sub: "Immutable Identity Ledger", color: "violet" },
                  { label: "x402", sub: "Atomic USDC micro-payments", color: "emerald" },
                ].map((item, i) => (
                  <div key={i} className={`p-6 rounded-xl border border-${item.color}-500/20 bg-${item.color}-500/5 space-y-2`}>
                    <div className={`text-2xl font-bold text-${item.color}-400`}>{item.label}</div>
                    <div className="text-sm text-gray-400">{item.sub}</div>
                  </div>
                ))}
              </div>
              <div className="pb-4 text-center text-xs text-gray-600 font-mono">
                Veklom Sovereign Lifecycle — Cappo decides, BYOS executes, Control Plane governs
              </div>
            </motion.div>
          </motion.div>
        </div>
