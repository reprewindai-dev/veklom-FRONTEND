...
export default function EEEVerificationPage() {
 ...
 {["Parse the Envelope and check required members, including enforcer.","Check eee_version against implemented versions.","Check hash_alg against the local allowlist.","Recompute envelope_hash using JCS Canonicalization and compare.","Resolve the issuer's public key using kid and validate at least one signature.","Check authority_window containment and every authority_chain expiration.","Check actual_effects ⊆ allowed_effects if grammar is supported.","Check budget.consumed ≤ budget.granted per dimension.","Check gate coverage: every decision references rules present in the bundle.","Report enforcement_mode. Conformance claims MUST be computed exclusively over fail-closed envelopes.","Report revocation_check.method: 'none' as REVOCATION_NOT_CHECKED.","Report a verdict: VALID, VALID_WITH_UNRESOLVED_REFS, or INVALID."
 ].map((step, idx) => (
