...
<span className="text-theme-accent">import</span> asyncio
<span className="text-theme-accent">from</span> veklom_vnp <span className="text-theme-accent">import</span> VNPNodeOracle
...
<span className="text-theme-accent">async def</span> <span className="text-yellow-200">monitor_api</span>():
 <span className="text-gray-500"># Execute STAMP RFC 8762 micro-session</span>
 result = <span className="text-theme-accent">await</span> oracle.probe(
 target_url=<span className="text-green-300">"https://api.tier1-provider.com/v1/chat"</span>,
 pad_tlv_bytes=<span className="text-theme-accent">2048</span>
 )
 
 <span className="text-gray-500"># Submit cryptographically signed timings to the VNP Mesh</span>
 <span className="text-theme-accent">await</span> oracle.submit_attestation(result)

<span className="text-theme-accent">if</span> __name__ == <span className="text-green-300">"__main__"</span>:
