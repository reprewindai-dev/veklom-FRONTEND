...
export default function CapabilityHome() {
  ...
  const trustSpine = ["Identity", "Capability", "Govern", "Execute", "Evidence", "Settle"];
  return (
    <section className="mx-auto max-w-7xl px-5 py-8 lg:px-10 lg:py-12">
      ...
      <div className="mt-10 rounded-2xl border border-cos-border bg-cos-surface2/55 p-4 shadow-cos-card lg:p-5"><div className="mb-4 flex items-center justify-between"><div><div className="font-mono text-[9px] uppercase tracking-[0.22em] text-cos-accent">One machine action</div><h2 className="mt-1 text-sm font-medium uppercase tracking-[0.16em] text-cos-text">Trust spine</h2></div><span className="font-mono text-[10px] text-cos-steel">IDENTITY → SETTLEMENT</span></div><div className="flex flex-wrap items-center gap-y-4">{trustSpine.map((stage, index) => <div key={stage} className="flex items-center"><div className="flex items-center gap-2 rounded-full border border-cos-border bg-cos-bg/50 px-3 py-2"><span className="h-1.5 w-1.5 rounded-full bg-cos-unknown shadow-[0_0_10px_rgba(107,114,128,0.7)]" /><span className="font-mono text-[10px] uppercase tracking-[0.14em] text-cos-steel">{stage}</span><ProofBadge status="Needs proof" /></div>{index < trustSpine.length - 1 && <span className="mx-2 hidden h-px w-5 bg-gradient-to-r from-cos-border via-cos-accent/45 to-cos-border sm:block" />}</div>)}</div></div>
