...
export default function ExecutePage() {
  ...
  async function readTargetState() {
    ...
    if (!lease.project) {
      const readbackResponse = { error: "mount project scope missing" };
    ...
    const result = await callCappo<JsonRecord>(
      endpoint(
        "GET",
        `/v1/capability/targets/${encodeURIComponent(targetRef)}/state?resource=${encodeURIComponent(resource)}&project=${encodeURIComponent(lease.project)}`,
        cappoBase,
      ),
    );
