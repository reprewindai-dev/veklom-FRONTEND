...
describe("CAPPO proxy boundary", () => {
  ...
  it("exchanges execution identity before forwarding without an internal CAPPO key", async () => {
    ...
    expect(fetchSpy.mock.calls[0]?.[0]).toBe("https://api.veklom.com/api/v1/auth/me");
    expect(fetchSpy.mock.calls[1]?.[0]).toBe("https://api.veklom.com/api/v1/auth/cappo-token");
    expect(fetchSpy.mock.calls[2]?.[0]).toBe("https://cappo.test/v1/exec");
    const upstreamHeaders = new Headers(fetchSpy.mock.calls[2]?.[1]?.headers);
  ...
  it("proxies the authenticated agents collection through CAPPO", async () => {
    ...
    expect(fetchSpy.mock.calls[2]?.[0]).toBe("https://cappo.test/api/v1/agents");
    const upstreamHeaders = new Headers(fetchSpy.mock.calls[2]?.[1]?.headers);
    expect(upstreamHeaders.get("x-api-key")).toBeNull();
