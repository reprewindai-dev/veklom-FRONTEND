...
export default function App() {
 ...
 return (
 <div id="lock-monitor-root" className="min-h-screen bg-theme-surface text-slate-100 font-sans selection:bg-theme-accent/30 selection:text-theme-accent/70">
 
 {/* HEADER SECTION */}
 <header id="dashboard-header" className="border-b border-theme-border bg-theme-surface backdrop-blur px-6 py-4 sticky top-0 z-50">
 <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
 <div className="flex items-center gap-3">
 <div className="p-2.5 bg-theme-surface2 border border-theme-border rounded-xl shadow-inner text-theme-accent">
 ...
 <div className="px-3 py-1.5 bg-theme-surface border border-theme-border rounded-lg flex items-center gap-2">
 <Server className="w-3.5 h-3.5 text-theme-accent" />
 <span className="text-slate-400">Redis-Node:</span>
 <span className="font-mono text-theme-accent/70 font-semibold bg-theme-surface2 px-1 py-0.5 rounded">6379-Active</span>
 </div>
 <div className="px-3 py-1.5 bg-theme-surface border border-theme-border rounded-lg flex items-center gap-2">
 ...
 <span className="font-mono text-theme-accent/70 font-medium">2026-06-05 13:07:52</span>
 </div>
 </div>
 </div>
 </header>
 ...
 <section id="datasource-config-panel" className="bg-theme-surface border border-theme-border rounded-2xl p-6 shadow-2xl relative overflow-hidden backdrop-blur-sm">
 <div className="absolute top-0 left-0 w-48 h-48 bg-theme-accent/5 rounded-full blur-3xl pointer-events-none"></div>
 
 <div className="space-y-5">
 <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-theme-border pb-4 gap-4">
 <div>
 ...
 ? 'bg-theme-surface to-emerald-500 text-slate-950 font-bold shadow-md'
 ...
 className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
 dataSourceMode === 'api'
 ? 'bg-theme-accent text-slate-950 font-bold shadow-md'
 : 'text-slate-400 hover:text-slate-100'
 }`}
 >
 ...
 {dataSourceMode === 'simulated' && (
 <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-center bg-theme-surface p-4 border border-theme-border rounded-xl animate-fade-in">
 ...
 <div className="text-xs text-right bg-theme-surface p-3 rounded-lg border border-theme-border font-mono w-full">
 ...
 <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-center bg-theme-surface p-4 border border-theme-border rounded-xl">
 <div className="md:col-span-8 space-y-1">
 <div className="flex items-center gap-2">
 <span className={`w-2 h-2 rounded-full ${apiConnectionStatus === 'connected' ? 'bg-theme-accent animate-pulse' : 'bg-rose-500'}`}></span>
 <span className={`text-xs font-semibold ${apiConnectionStatus === 'connected' ? 'text-theme-accent' : 'text-rose-400'} tracking-wide uppercase`}>
 ...
 <div className="bg-theme-surface p-2.5 rounded-lg border border-theme-border font-mono text-center">
 <div className="text-slate-500 text-[10px] uppercase">Listener Status</div>
 <div className="text-theme-accent font-mono font-semibold text-xs mt-1 truncate">
 {apiConnectionStatus === 'connected' ? 'Connected (Port 3000)' : 'Reconnecting...'}
 </div>
 </div>
 <div className="flex items-center gap-1 bg-theme-surface p-1.5 rounded-lg border border-theme-border text-[11px] font-mono select-all">
 <span className="text-slate-500 px-1 truncate">POST Endpoint</span>
 <input 
 type="text" 
 readOnly 
 value={`${origin}/api/logs`} 
 className="bg-transparent border-0 outline-0 ring-0 text-theme-accent/70 w-full font-mono text-right text-[10px]"
 />
 <button 
 onClick={() => {
 ...
 className="p-1 text-slate-400 hover:text-white rounded bg-theme-surface transition-all font-sans cursor-pointer"
 ...
 <div className="bg-theme-surface border border-theme-border rounded-xl overflow-hidden p-4">
 ...
 <div className="flex bg-theme-surface p-1 rounded-lg border border-theme-border text-[10px]">
 ...
 integrationTab === tab ? 'bg-theme-surface2 text-theme-accent border border-theme-border font-bold' : 'text-slate-400 hover:text-white'
 }`}
 >
 {tab}
 </button>
 ...
 <div className="bg-theme-surface p-3 rounded-lg border border-theme-border font-mono text-[11px] text-slate-300 max-h-[160px] overflow-y-auto relative">
 {integrationTab === 'curl' && (
 <pre className="whitespace-pre-wrap">{`curl -X POST \\
 -H"Content-Type: application/json" \\
 -d '{"key":"lock:payment_charge:115","operation":"EVALSHA","type":"success","latency": 1.45,"clientId":"client:remote_cl_09","duration": 15000,"message":"Real lock acquired dynamically via cURL shell stub"
 }' \\
 \${origin}/api/logs`}</pre>
 )}
 {integrationTab === 'node' && (
...
def lock_telemetry(key_name, client_id, lease_ms):
 ...
 try:
 requests.post('\${origin}/api/logs', json={"key": key_name,"operation":"EVALSHA","type":"success" if success else"failed","latency": round(elapsed_ms, 2),"clientId": client_id,"duration": lease_ms,"message": f"Python lock client acquired key {key_name} dynamically."
 }, timeout=0.5)
 except Exception:
 pass`}</pre>
 )}
...
func LogEvent(key string, latency float64, success bool) {
 ...
 payload := map[string]interface{}{"key": key,"operation":"EVALSHA","type": status,"latency": latency,"message":"Golang lock client performance callback",
 }
 ...
 className="absolute top-2 right-2 p-1.5 rounded bg-theme-surface border border-theme-border hover:bg-theme-surface text-slate-400 hover:text-white transition-all flex items-center gap-1 text-[10px] cursor-pointer"
 ...
 {dataSourceMode === 'pasted' && (
 <div className="space-y-4 bg-theme-surface p-4 border border-theme-border rounded-xl animate-fade-in">
 <div className="space-y-1">
 <div className="flex items-center gap-2">
 <span className="w-2 h-2 rounded-full bg-theme-accent animate-ping block"></span>
 <span className="text-xs font-semibold text-theme-accent tracking-wide uppercase">Raw Log Ingestion Mode</span>
 </div>
...
[
 ...
 className="w-full text-xs font-mono bg-theme-surface border border-theme-border focus:border-theme-border rounded-xl p-3 outline-none text-slate-300"
 />
 ...
 className="px-4 py-2 bg-theme-surface text-white rounded-lg text-xs font-semibold hover: hover: transition-all active:scale-95 shadow-md flex items-center gap-1 cursor-pointer"
 >
 ...
 <section id="control-bar-card" className="bg-theme-surface border border-theme-border rounded-2xl p-5 shadow-2xl relative overflow-hidden backdrop-blur-sm">
 <div className="absolute top-0 right-0 w-32 h-32 bg-theme-accent/5 rounded-full blur-3xl pointers-events-none"></div>
 ...
 <div className="flex items-center bg-theme-surface border border-theme-border p-1 rounded-xl">
 ...
 <div className="grid grid-cols-4 p-1 bg-theme-surface border border-theme-border rounded-xl">
 ...
 medium: 'text-theme-accent border-theme-border hover:bg-theme-surface2 active:bg-theme-surface2',
 ...
 medium: 'bg-theme-accent text-slate-950 font-bold',
 ...
 className="flex items-center gap-1.5 px-3 py-2 bg-theme-surface border border-theme-border hover:border-theme-border text-theme-accent rounded-lg text-xs transition-all active:scale-95 text-nowrap font-medium"
 title="EVALSHA Acquire lock script on Redis"
 >
 <Zap className="w-3.5 h-3.5 text-theme-accent fill-cyan-400/20" />
 ...
 {/* SUCCESS RATE CARD */}
 <div id="stat-card-success-rate" className="bg-theme-surface border border-theme-border p-5 rounded-2xl relative overflow-hidden flex flex-col justify-between h-[120px]">
 ...
 {/* SCRIPT LATENCY CARD */}
 <div id="stat-card-latency" className="bg-theme-surface border border-theme-border p-5 rounded-2xl flex flex-col justify-between h-[120px]">
 <div className="flex items-start justify-between">
 <div>
 <p className="text-xs text-slate-400 font-semibold tracking-wider uppercase">Avg Script Latency</p>
 <div className="flex items-baseline gap-1 mt-1">
 <h3 className="text-3xl font-bold tracking-tight text-theme-accent">
 {currentAvgLatency}
 </h3>
 <span className="text-xs text-theme-accent font-mono font-semibold">ms</span>
 </div>
 </div>
 <div className="p-2 bg-theme-surface2 border border-theme-border text-theme-accent rounded-xl">
 <Activity className="w-5 h-5" />
 </div>
 </div>

 <div className="flex items-center justify-between text-[11px] font-mono text-slate-400 border-t border-theme-border pt-1.5 mt-auto">
 <div>Min: <span className="text-emerald-400 font-semibold">{history.length > 0 ? Math.min(...history.map(h => h.minLatency)) : 0}ms</span></div>
 <div className="h-2.5 w-[1px] bg-theme-surface"></div>
 <div>Max Load: <span className="text-rose-400 font-semibold">{currentMaxLatency}ms</span></div>
 </div>
 </div>

 {/* ACTIVE LOCK COUNTER */}
 <div id="stat-card-active-leases" className="bg-theme-surface border border-theme-border p-5 rounded-2xl flex flex-col justify-between h-[120px]">
 <div className="flex items-start justify-between">
 <div>
 ...
 <div className="flex items-center justify-between text-[11px] text-slate-400 border-t border-theme-border pt-1.5 mt-auto">
 ...
 {/* DATABASE THROUGHPUT */}
 <div id="stat-card-throughput" className="bg-theme-surface border border-theme-border p-5 rounded-2xl flex flex-col justify-between h-[120px]">
 <div className="flex items-start justify-between">
 <div>
 ...
 <span className="text-xs text-theme-accent font-semibold">ops/sec</span>
 </div>
 </div>
 <div className="p-2 bg-theme-surface2 border border-theme-border text-theme-accent rounded-xl">
 ...
 <div id="chart-visualizer-container" className="lg:col-span-7 bg-theme-surface border border-theme-border rounded-2xl p-6 shadow-xl flex flex-col gap-6">
 <div className="flex items-center justify-between border-b border-theme-border pb-4">
 <div className="flex items-center gap-2">
 <Activity className="w-4 h-4 text-theme-accent animate-pulse" />
 <h2 className="text-sm font-bold uppercase tracking-wider text-white">Live Stream Analytics</h2>
 </div>

 {/* Chart Tabs selector */}
 <div className="flex bg-theme-surface p-0.5 rounded-lg border border-theme-border">
 <button
 id="tab-btn-latency"
 onClick={() => setCurrentTab('latency')}
 className={`px-3 py-1 text-xs rounded transition-all font-semibold ${
 currentTab === 'latency' ? 'bg-theme-accent text-slate-950 shadow' : 'text-slate-400 hover:text-white'
 }`}
 >
 Latency Streams
 </button>
 ...
 className={`px-3 py-1 text-xs rounded transition-all font-semibold ${
 currentTab === 'throughput' ? 'bg-theme-accent text-slate-950 shadow' : 'text-slate-400 hover:text-white'
 }`}
 >
 Acquisition Success
 </button>
 ...
 className={`px-3 py-1 text-xs rounded transition-all font-semibold ${
 currentTab === 'distribution' ? 'bg-theme-accent text-slate-950 shadow' : 'text-slate-400 hover:text-white'
 }`}
 >
 Tail Spill
 </button>
 ...
 {selectedChartPoint && (
 <div className="bg-theme-surface border border-theme-border rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-fade-in text-xs font-mono relative overflow-hidden shadow-2xl">
 <div className="absolute top-0 right-0 px-2 py-0.5 bg-theme-surface2 text-theme-accent font-bold uppercase tracking-wider text-[8px] border-b border-l border-theme-border rounded-bl select-none">
 ...
 Avg Latency: <span className="text-theme-accent font-bold">{selectedChartPoint.avgLatency}ms</span> | Peak Spike: <span className="text-rose-400 font-bold">{selectedChartPoint.maxLatency}ms</span> | successes: <span className="text-emerald-400 font-bold">{selectedChartPoint.successes}</span> | fails: <span className="text-rose-400 font-bold">{selectedChartPoint.failures}</span>
 </p>
 </div>
 ...
 ? 'bg-theme-surface2 border-theme-border text-theme-accent font-extrabold'
 : 'bg-theme-surface border-theme-border text-slate-300 hover:bg-theme-surface hover:text-white'
 }`}
 ...
 className="p-1.5 hover:bg-rose-950/40 border border-theme-border hover:border-rose-900 rounded-lg text-slate-400 hover:text-white transition-all ml-1 cursor-pointer"
 ...
 return (
 <div className={`bg-theme-surface border p-3.5 rounded-xl shadow-2xl font-mono text-xs max-w-[280px] transition-all ${isPointLocked ? 'border-theme-border shadow-[0_0_15px_rgba(6,182,212,0.15)] bg-theme-surface' : 'border-theme-border'}`}>
 <div className="flex items-center justify-between border-b border-theme-border pb-1.5 mb-2">
 <span className="text-white font-bold">{label}</span>
 {isPointLocked && (
 <span className="text-[9px] bg-theme-surface2 text-theme-accent px-1.5 py-0.2 rounded font-semibold animate-pulse">LOCKED</span>
 )}
 </div>
 <div className="space-y-1">
 {payload.map((p, i) => (
 ...
 {isPointLocked ? (
 <div className="mt-2.5 border-t border-theme-border pt-2 text-[10px] font-sans text-theme-accent/90 leading-normal">
 ...
 <div className="mt-2.5 border-t border-theme-border pt-2 text-[10px] text-slate-500 font-sans flex items-center gap-1.5">
 ...
 <div className="mt-2 border-t border-theme-border pt-1.5 flex justify-between font-bold text-theme-accent">
 ...
 <div className="bg-theme-surface h-2.5 rounded-full overflow-hidden border border-theme-border">
 <div 
 ...
 <div className="w-full max-w-[260px] bg-theme-surface border border-theme-border p-4 rounded-xl text-xs space-y-2.5">
 <div className="flex items-center gap-1.5 text-theme-accent font-semibold uppercase tracking-wider text-[10px]">
 <Info className="w-4 h-4 text-theme-accent flex-shrink-0" />
 ...
 <p className="text-slate-400 leading-relaxed">
 <strong className="text-theme-accent">Normal Operations:</strong> Moderate lease contentions are occurring. Single threading limits do not saturate context, resulting in highly stable atomic latency.
 </p>
 )}
 ...
 <div className="grid grid-cols-3 gap-2 text-center text-[11px] bg-theme-surface rounded-xl p-3 border border-theme-border">
 <div>
 <span className="text-slate-500 font-medium block">Total EVAL SHA runs</span>
 <span className="font-mono text-theme-accent font-bold text-sm block mt-0.5">{stats.cumulativeAcquires}</span>
 </div>
 ...
 <div id="lua-script-inspector" className="lg:col-span-5 bg-theme-surface border border-theme-border rounded-2xl p-6 shadow-xl flex flex-col gap-4">
 <div className="flex items-center justify-between border-b border-theme-border pb-4">
 <div className="flex items-center gap-2">
 ...
 <div className="flex bg-theme-surface p-0.5 rounded-lg border border-theme-border select-none">
 ...
 selectedScriptKey === key ? 'bg-theme-accent text-slate-950 shadow' : 'text-slate-400 hover:text-white'
 }`}
 >
 {key}
 </button>
 ...
 <div className="bg-theme-surface border border-theme-border rounded-xl p-4 font-mono text-[11px] leading-relaxed relative flex flex-col justify-between overflow-x-auto min-h-[220px]">
 <div className="absolute top-2 right-2 flex items-center gap-1.5 opacity-40 select-none pointer-events-none">
 <span className="text-[9px] font-semibold bg-theme-surface text-slate-400 px-1.5 py-0.5 rounded border border-theme-border">redis-lua</span>
 </div>
 ...
 isSelected 
 ? 'bg-theme-surface2 border-l-2 border-theme-border text-theme-accent/70' 
 : isHovered 
 ...
 item.perf === 'O(1)' ? 'bg-emerald-950/60 border border-emerald-900/40 text-emerald-400' : 'bg-theme-surface text-slate-500'
 }`}>
 {item.perf}
 </span>
 )}
 ...
 <div id="inspector-line-explanation" className="bg-theme-surface border border-theme-border p-4 rounded-xl flex flex-col gap-2 min-h-[90px]">
 ...
 <div className="text-[10px] text-slate-500 bg-theme-surface border border-theme-border p-3 rounded-lg leading-relaxed flex items-start gap-2 select-none">
 ...
 {/* LOG CONSOLE (8 col) */}
 <div id="terminal-logs-card" className="lg:col-span-8 bg-theme-surface border border-theme-border rounded-2xl p-6 shadow-xl flex flex-col justify-between gap-4 h-[350px]">
 ...
 className="bg-theme-surface border border-theme-border rounded-lg pl-8 pr-3 py-1 text-xs text-white max-w-[150px] placeholder:text-slate-600 focus:outline-none focus:border-theme-border"
 />
 {searchLogQuery && (
 <button 
 ...
 className="bg-theme-surface border border-theme-border rounded-lg px-2.5 py-1 text-xs text-slate-300 focus:outline-none focus:border-theme-border cursor-pointer"
 >
 <option className="bg-theme-surface text-white" value="timestamp_newest">Timestamp (Newest)</option>
 <option className="bg-theme-surface text-white" value="latency_high_low">Latency (High to Low)</option>
 <option className="bg-theme-surface text-white" value="latency_low_high">Latency (Low to High)</option>
 </select>
 ...
 id="btn-clear-logs"
 onClick={() => setLogs([])}
 className="p-1 px-1.5 text-[10px] font-mono border border-theme-border rounded text-slate-500 hover:text-slate-300 hover:border-theme-border transition"
 ...
 logFilter === f 
 ? 'bg-theme-surface text-white font-bold border border-theme-border' 
 : 'bg-theme-surface text-slate-500 hover:text-slate-300 border border-transparent'
 }`}
 >
 {f}
 </button>
 ...
 <div className="bg-theme-surface border border-theme-border rounded-xl flex-1 font-mono text-[10px] overflow-y-auto p-4 space-y-1 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-slate-950">
 {filteredLogs.length > 0 ? (
 filteredLogs.map((log) => {
 let badgeColor = 'text-slate-400';
 let logPrefixBg = 'bg-theme-surface text-slate-400';
 ...
 badgeColor = 'text-theme-accent';
 logPrefixBg = 'bg-theme-surface2 border border-theme-border text-theme-accent';
 ...
 <div 
 key={log.id} 
 className={`flex gap-2.5 py-1 border-b border-theme-border hover:bg-theme-surface rounded px-1.5 transition-all ${
 isExactHighlight 
 ? 'bg-theme-surface2 border-l-2 border-theme-border shadow-[inset_1px_0_12px_rgba(6,182,212,0.15)] ring-1 ring-theme-accent' 
 : ''
 }`}
 >
 ...
 <span className="text-[8px] text-theme-accent bg-theme-surface2 border border-theme-border rounded px-1">
 {log.latency}ms
 </span>
 )}
 </div>
 ...
 <div id="keyspace-lease-card" className="lg:col-span-4 bg-theme-surface border border-theme-border rounded-2xl p-6 shadow-xl flex flex-col justify-between gap-4 h-[350px]">
 ...
 <div className="flex-1 bg-theme-surface border border-theme-border rounded-xl p-4 overflow-y-auto space-y-3">
 ...
 return (
 <div key={lock.key} className="bg-theme-surface border border-theme-border rounded-xl p-3 space-y-2 relative transition-all hover:bg-theme-surface">
 
 <div className="flex items-center justify-between">
 <div className="flex items-center gap-1.5 overflow-hidden">
 <span className="h-1.5 w-1.5 rounded-full bg-amber-500"></span>
 <span className="text-[11px] font-mono text-theme-accent/70 font-semibold truncate" title={lock.key}>
 {lock.key}
 </span>
 </div>
 ...
 className="text-slate-500 hover:text-theme-accent p-1 rounded hover:bg-theme-surface2 transition-all border border-transparent hover:border-theme-border"
 ...
 <div className="h-7 w-16 bg-theme-surface border border-theme-border rounded px-1 flex items-center justify-center overflow-hidden flex-shrink-0" title="Lock Remaining Lease sawtooth activity graph">
 <ResponsiveContainer width="100%" height="100%">
 ...
 <div className="bg-theme-surface rounded-full h-1.5 border border-theme-border overflow-hidden">
 <div 
 ...
 <div className="text-[10px] text-slate-500 bg-theme-surface p-2.5 border border-theme-border rounded-xl text-center font-mono">
 ...
 {/* FOOTER CREDITS */}
 <footer id="global-credits" className="text-center py-8 text-xs text-slate-600 border-t border-theme-border bg-theme-surface">
