...
// â”€â”€ Pipeline Definitions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Threat Scenarios â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const SEKED_STYLES: Record<SEKEDDirective, string> = {
 ...
 FORTIFY: 'text-theme-accent bg-theme-accent/10 border-theme-border',
...
export default function SimulatorPanel() {
 ...
 push(`EXEC_START â€”"${scenario.label}" â€” agent: ${agentModel}`);
 ...
 push(`âš  Phase ${finalPhase + 1}: QUARANTINE â€” ${receipt.reason}`);
 push(`â†’ HTTP 202 Accepted returned. Quarantine ID: ${receipt.quarantine_id}`);
 ...
 push(`âš  Phase ${finalPhase + 1}: BLOCK â€” ${receipt.reason}`);
 push(`âœ• Execution HALTED at Phase ${finalPhase + 1}.`);
 setPipeline(prev => prev.map((s, idx) => idx === finalPhase ? { ...s, status: 'blocked' } : s));
 setSeked('HALT');
 setBlockedCount(c => c + 1);
 } else {
 // Success
 push(`â†’ Phase 6: Executing â€” live sandboxed run`);
 push(`â†’ Phase 7: Evidence â€” Merkle proof generated on PGL: ${receipt.evidence_chain_id}`);
 push(`â†’ Phase 8: Audit â€” Trust score updated (${receipt.new_trust_score})`);
 ...
 setSeked('EXECUTE');
 push(`â†’ EAT issued: ${receipt.evidence_chain_id}`);
 push(`EXEC_COMPLETE â€” all 9 gates cleared âœ“ â€” evidence sealed`);
 setEatCount(c => c + 1);
 setRunCount(c => c + 1);
 }
 ...
 return 'border-theme-border bg-[#101010]';
 };
 ...
 return 'border-theme-border bg-[#0A0A0A] text-slate-500';
 };
 ...
 if (line.includes('âœ•') || line.includes('âš  BLOCK') || line.includes('HALTED')) return 'text-[#FF003C]';
 if (line.includes('âš ')) return 'text-[#FFAB00]';
 if (line.includes('âœ“') || line.includes('EAT') || line.includes('COMPLETE')) return 'text-[#00FF66]';
 return 'text-slate-400';
 };
 ...
 {/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ LEFT: Config Panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
 ...
 disabled={isRunning}
 rows={4}
 className="w-full bg-[#070b12] border border-theme-border rounded-lg px-3 py-2.5 text-xs text-slate-300 font-mono resize-none focus:outline-none focus:border-[#FFB800]/40 placeholder-slate-600 disabled:opacity-50 transition-colors"
 placeholder="Describe what the agent should doâ€¦"
 />
 </div>
 ...
 className="w-full bg-[#070b12] border border-theme-border rounded-lg px-3 py-2 text-xs text-slate-300 font-mono focus:outline-none focus:border-[#FFB800]/40"
 >
 <option value="gemini-pro">Gemini Pro â€” VNP 94 Â· T1</option>
 <option value="gpt-4o">GPT-4o â€” VNP 91 Â· T1</option>
 <option value="claude-3">Claude 3.7 â€” VNP 89 Â· T2</option>
 <option value="ollama-local">Ollama Local â€” VNP 98 Â· T1</option>
 <option value="deepseek">DeepSeek R1 â€” VNP 82 Â· T3</option>
 </select>
 </div>
 <div>
 ...
 : 'bg-[#070b12] border-theme-border text-slate-500 hover:text-slate-300'
 }`}
 >
 {l}
 </button>
 ...
 { tool: 'exec_shell', allowed: false, note: 'BLOCKED â€” all environments' },
 ...
 <div key={p.tool} className="flex items-center justify-between py-1.5 border-b border-theme-border last:border-0">
 <div>
 ...
 <div key={r.id} className="flex items-center justify-between py-1.5 border-b border-theme-border last:border-0">
 <div>
 ...
 : 'border-theme-border bg-[#070b12] hover:border-theme-border'
 }`}
 >
 ...
 onClick={run}
 disabled={isRunning}
 className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-[#FFB800] hover:bg-[#E0A100] disabled:bg-theme-surface disabled:text-slate-500 text-slate-950 font-bold text-sm transition-all shadow-[0_0_24px_rgba(255,184,0,0.12)] hover:shadow-[0_0_32px_rgba(255,184,0,0.22)]"
 >
 <Play className="w-4 h-4" />
 {isRunning ? 'Runningâ€¦' : 'Run Enforcement'}
 </button>
 <button
 onClick={reset}
 disabled={isRunning}
 className="px-4 py-3 rounded-xl bg-theme-surface hover:bg-theme-surface border border-theme-border text-slate-400 transition-all disabled:opacity-40"
 >
 ...
 {/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ CENTER: Pipeline â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
 ...
 Every agent action passes every gate â€” cryptographically enforced, in order, every single time.
 </p>
 </div>
 ...
 {/* connector top */}
 {idx > 0 && (
 <div className={`absolute left-[27px] top-0 w-0.5 h-3 -mt-3 ${done ? 'bg-[#00FF66]/30' : 'bg-theme-surface'}`} />
 )}
 ...
 {active && (
 <div className="w-20 h-1 bg-theme-surface rounded-full overflow-hidden">
 ...
 {/* connector bottom */}
 {!isLast && (
 <div className={`absolute left-[27px] bottom-0 w-0.5 h-3 -mb-3 z-10 ${done ? 'bg-[#00FF66]/30' : 'bg-theme-surface'}`} />
 )}
 {/* gap spacer */}
 ...
 Execution Authorization Token â€” Issued
 </span>
 </div>
 <button
 ...
 MERKLE ROOT Â· {Math.random().toString(16).slice(2, 42)} Â· Base L2 Block #1,442,88{eatCount % 10}
 </div>
 </div>
 )}
 </div>

 {/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ RIGHT: Telemetry + Zero-Trust â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
 <div className="flex flex-col gap-4">
 ...
 {/* Terminal */}
 <div className="bg-theme-surface border border-theme-border rounded-xl overflow-hidden">
 <div className="flex items-center justify-between px-4 py-3 border-b border-theme-border bg-[#070b12]">
 <div className="flex items-center gap-2">
 ...
 ? <span className="text-slate-500">Select a scenario and press Run Enforcementâ€¦</span>
 : terminalLines.map((line, i) => (
 <div key={i} className={termLineClass(line)}>{line}</div>
 ))
 }
 {isRunning && <div className="text-[#FFB800] animate-pulse">â–ˆ</div>}
 </div>
 </div>

 {/* Enforcement Ledger */}
 <div className="bg-theme-surface border border-theme-border rounded-xl p-4 relative">
 ...
 <div key={z.label} className="flex items-center justify-between py-1 border-b border-theme-border last:border-0">
 ...
 {/* What this is */}
 <div className="bg-[#070b12] border border-theme-border rounded-xl p-4">
 ...
 <p>Veklom decides what agents are allowed to do â€” before they do it â€” and verifies whether the systems they call are trustworthy enough to receive the action.</p>
