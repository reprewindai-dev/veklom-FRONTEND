...
export default function SpecPanel() {
 ...
 const [simLog, setSimLog] = useState<string[]>(["Tokio async thread-pool booted successfully.","DBPool max_connections(12) min_connections(3) active.","Row Level Security policies loaded inside transaction buffers."
 ]);
...
-- tx.execute(sqlx::query("SELECT * FROM compliance_audit_log;")).await?;`;
 ...
 activeTab ==="cli" ?"sqlx-cli Migrations Instructions (.env)" :"Row Level Security (RLS) isolation.sql";
 ...
 <div className="bg-[#121924] border border-theme-border rounded-2xl p-2 flex gap-1 font-mono text-[10px]">
 <button
 onClick={() => setViewMode("simulation")}
 className={`flex-1 py-2.5 rounded-xl font-extrabold cursor-pointer transition ${
 viewMode ==="simulation" 
 ?"bg-[#1d273a] text-emerald-400 border border-theme-border" 
 ...
 <div className="bg-[#0b1017] border border-theme-border rounded-2xl p-5 space-y-4">
 <div className="flex items-center gap-1.5 border-b border-theme-border pb-2.5">
 ...
 :"bg-theme-surface border border-theme-border text-slate-400 hover:bg-theme-surface hover:text-white"
 }`}
 >
 ...
 <span className="text-[9px] text-theme-accent font-bold">Isolation</span>
 </button>
 </nav>
 </div>

 {/* Informational connection counter stats card */}
 <div className="bg-theme-surface border border-theme-border p-4 rounded-xl space-y-3 font-mono text-[10px]">
 ...
 <div className="bg-[#090d13] border border-theme-border rounded-2xl p-5 space-y-5 h-[535px] flex flex-col justify-between font-mono text-[11px]">
 
 {/* Header Dashboard stats bar */}
 <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-theme-border pb-4">
 ...
 disabled={isFlooding}
 className="p-3 bg-theme-surface from-emerald-600 hover:from-emerald-500 hover: disabled:opacity-40 text-white rounded-xl text-xs font-black transition duration-200 flex items-center gap-2 shadow-lg shadow-emerald-950/20 active:scale-95 cursor-pointer selection:bg-transparent"
 >
 ...
 {slots.map((slot) => {
 let statusColor ="border-theme-border bg-[#0d131f]/20 text-slate-500";
 let lightColor ="bg-theme-surface";
 ...
 <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center text-[10px] uppercase font-bold text-slate-500 pt-1 border-t border-theme-border">
 <div className="p-2.5 bg-[#0e1420]/50 border border-theme-border rounded-xl">
 ...
 <span className="text-white text-sm block mt-1 font-black text-theme-accent">{throughputCounter} RPS</span>
 </div>
 </div>
 ...
 <div className="bg-[#05060a] border border-theme-border p-3 rounded-xl h-[120px] overflow-y-auto font-mono text-[9.5px]/relaxed text-slate-400 space-y-1 select-text custom-scrollbar">
 {simLog.map((log, idx) => {
 let badge ="text-slate-500";
 if (log.includes("ACQUIRED")) badge ="text-emerald-400";
 if (log.includes("SECURITYCHECK") || log.includes("RLS")) badge ="text-theme-accent";
 if (log.includes("BURST") || log.includes("FLOOD")) badge ="text-yellow-400 font-extrabold animate-pulse";
 return (
 <div key={idx} className={`${badge} break-all border-b border-theme-border pb-0.5`}>
 {log}
 </div>
 );
 })}
 ...
 <div className="bg-[#090d13] border border-theme-border rounded-2xl overflow-hidden flex flex-col h-[535px]">
 {/* Header bar */}
 <div className="bg-[#121822] border-b border-theme-border px-5 py-3.5 flex items-center justify-between">
 ...
 className="flex items-center gap-1.5 px-3 py-1.5 bg-theme-surface hover:bg-theme-surface text-slate-200 hover:text-white rounded text-xs transition duration-200 cursor-pointer font-bold font-mono text-[10px]"
 >
 {copied === activeTab ? (
 <>
 ...
 {/* Footer info bar */}
 <div className="bg-theme-surface border-t border-theme-border px-5 py-3 text-xs text-slate-500 flex items-center justify-between">
