...
export default function NetworkTopologyPanel() {
 ...
 {/* LEFT: node map */}
 <div className="lg:col-span-8 flex flex-col bg-theme-surface from-[#080d15] to-[#04070a] border border-theme-border rounded-2xl p-5 relative overflow-hidden h-[570px]">
 <div className="flex items-center justify-between border-b border-theme-border pb-3">
 <div className="space-y-1.5">
 <div className="flex items-center gap-2">
 <Radio className="w-5 h-5 text-theme-accent" />
 <span className="text-sm font-sans tracking-wide text-white/90">
 VNP Beacon Topology
 </span>
 </div>
 <div className="flex items-center gap-2">
 <span className="text-[9px] uppercase tracking-widest border px-2 py-0.5 rounded text-slate-300 bg-theme-surface border-theme-border">
 {meshLabel}
 </span>
 ...
 <div className="flex items-center gap-2 text-[10px] text-theme-accent/50 uppercase tracking-widest pb-2 border-b border-theme-border">
 <Terminal className="w-3.5 h-3.5 text-theme-accent" />
 <span>Beacon event log (as reported)</span>
 </div>
 <div className="h-[70px] overflow-y-auto text-[10px] leading-relaxed text-slate-300 space-y-1.5 p-2 bg-[#03070c]/50 rounded border border-theme-border">
 {eventsLog.length === 0 ? (
 <div className="text-theme-accent/30 italic">No events reported by the beacon.</div>
 ) : (
 eventsLog.map((log, idx) => (
 <div key={idx} className="text-theme-accent/70/70 break-all flex gap-2">
 <span className="text-theme-accent/30 shrink-0">❯</span>
 <span>{log}</span>
 </div>
 ))
 )}
 ...
 <div className="bg-[#080d15]/50 border border-theme-border rounded-2xl p-5 flex items-center justify-center text-theme-accent/50 text-xs italic h-[280px]">
 No node telemetry available.
 </div>
 )}

 <div className="bg-theme-surface from-[#0b1219]/90 to-[#03070c]/90 border border-theme-border rounded-2xl p-4 flex-1 flex flex-col min-h-[160px]">
 ...
 {ledgerFeed.length === 0 ? (
 <div className="flex h-full items-center justify-center text-[10px] text-theme-accent/30 italic">
 ...
 <div
 key={tx.id ?? idx}
 className="p-3 bg-[#03070c]/60 border border-theme-border rounded-lg flex items-center justify-between"
 >
...
function NodeInspector({ node }: { node: VnpTopologyNode }) {
 ...
 return (
 <div className="bg-theme-surface from-[#080d15]/90 to-[#03070c]/90 border border-theme-border rounded-2xl p-5 space-y-4">
 <div className="flex items-start gap-3 border-b border-theme-border pb-4">
 <div className="p-2 rounded-lg border bg-theme-surface2 border-theme-border">
 <Server className="text-theme-accent w-5 h-5" />
 </div>
 <div className="flex-1">
 <span className="text-[9px] uppercase tracking-[0.2em] text-theme-accent/50 block mb-0.5">
 VNP Node
 </span>
 ...
 <dt className="uppercase tracking-widest text-[9px] text-theme-accent/50 shrink-0">
 {label}
 </dt>
