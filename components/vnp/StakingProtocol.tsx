...
export default function StakingProtocol({ apis = [] }: StakingProtocolProps) {
 const { data: stakingRoute, mutate: mutateStakingRoute } = useSWR<StakingRouteState>("/staking-protocol/state",
 fetcher,
 ...
 setStakeResult(null);
 try {
 const res = await api<{ success: boolean; new_balance: number; volume: number; yesPrice: number; noPrice: number }>("/api/v1/benchmarks/staking/stake",
 ...
 { label:"Active APIs", value: protocolStats.activeApis != null ? String(protocolStats.activeApis) :"0", icon: Server, color:"text-theme-accent" },
...
{`{"version": 1,"api_id":"0x...","stake_usdc":"0.001","epoch_hint": ${protocolStats.epochsProcessed},"agent":"did:veklom:agent-123","nonce":"...","signature":"0x..."
}`}
 </pre>
 </div>
 <div>
 ...
 {["Protocol target: agent attaches micro-stake ($0.001 USDC) to x402 payment header","Protocol target: off-chain aggregator batches outcomes per epoch","Needs settlement proof: yield from provider bond after SLA success","Needs settlement proof: slash/refund after verified SLA failure","Needs on-chain proof: periodic net-balance settlement on Base L2",
 ].map((step, i) => (
 ...
 <div className="font-mono text-sm text-theme-accent">{v.diversityScore.toFixed(2)}</div>
