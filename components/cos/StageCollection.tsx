"use client";
import { Pillar } from"./SectionPillars";
...
export function ChainState({ label, proof, detail }: { label: string; proof: ProofStatus; detail: string }) { return <div className="rounded-xl border border-cos-border bg-cos-bg/40 p-4"><div className="flex items-center justify-between gap-3"><span className="font-mono text-xs uppercase tracking-[.12em] text-cos-text">{label}</span><ProofBadge status={proof} /></div><p className="mt-3 text-xs leading-5 text-cos-muted">{detail}</p></div>; }
