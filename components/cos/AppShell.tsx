...
type DependencyHealth = {
  status?: string;
  dependencies?: Array<{ name?: string; state?: string; latency_ms?: number | null }>;
};
...
function runtimeObservationStatus(health: DependencyHealth | null): ProofStatus {
  if (!health) return "Needs proof";
  const state = (health.status || "").toLowerCase();
  if (state === "healthy") return "Present";
  if (state === "degraded" || state === "unconfigured" || state === "unavailable") return "Degraded";
  return "Needs proof";
}
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    import("@/lib/api").then(({ api }) => {
      if (disposed) return;
      const fetchHealth = () =>
        api.get<DependencyHealth>("/health/dependencies")
          .then(setHealth)
          .catch(() => setHealth(null));
      const fetchBalance = () =>
        api.get<WalletBalance>("/api/v1/wallet/balance")
          .then(setBalance)
  ...
  const runtimeStatus = runtimeObservationStatus(health);
        ...
        <div className="relative z-10 flex min-h-screen w-full flex-col">
          <header className="relative flex min-h-[76px] items-center justify-between gap-4 border-b border-cos-border/80 bg-cos-bg/70 px-4 shadow-[0_12px_35px_-28px_rgba(0,229,255,0.8)] backdrop-blur-2xl lg:px-7">
            ...
            <div className="flex items-center gap-5">
              ...
              <span className="hidden rounded border border-cos-accent/25 px-2.5 py-1.5 font-mono text-[9px] uppercase tracking-[0.18em] text-cos-steel lg:inline">Machine-to-Machine Trust Infrastructure</span>
            </div>
            <div className="flex items-center gap-2 text-xs">
              ...
              <div className="hidden items-center gap-2 rounded-full border border-cos-border bg-cos-surface2/40 px-3 py-2 text-cos-muted md:flex">
                <Cpu size={14} className={runtimeStatus === "Degraded" ? "text-cos-warn" : health ? "text-cos-accent" : "text-cos-steel"} />
                Runtime <ProofBadge status={runtimeStatus} />
              </div>
              <div className="hidden items-center gap-2 rounded-full border border-cos-border bg-cos-surface2/40 px-3 py-2 text-cos-muted xl:flex">
                <ShieldCheck size={14} className="text-cos-steel" />{identity}
              </div>
              {balance && (
                <div className="hidden items-center gap-2 rounded-full border border-cos-border bg-cos-surface2/40 px-3 py-2 font-mono text-[10px] text-cos-accent xl:flex">
                  ${(balance.balance_usd ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 })} {balance.currency || "USD"}
                </div>
              )}
              <button onClick={() => setPaletteOpen(true)} className="rounded-full border border-cos-border bg-cos-surface2/50 p-2.5 text-cos-steel transition hover:border-cos-accent/50 hover:text-cos-accent" aria-label="Open command palette">
                <Command size={16} />
              </button>
