  ...
  return (
    ...
    {open && <motion.div initial={reduceMotion ? false : { opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} exit={reduceMotion ? undefined : { opacity: 0, y: 28 }} transition={{ duration: reduceMotion ? 0 : 0.2 }} className="fixed inset-x-0 bottom-0 z-40 border-t border-cos-accent/25 bg-cos-bg/95 shadow-[0_-20px_80px_-35px_rgba(0,229,255,0.6)] backdrop-blur-xl">
      <div className="mx-auto max-w-7xl p-4">
        ...
        <div className="rounded-lg border border-cos-border bg-cos-bg/30 p-4"><p className="font-mono text-xs text-cos-muted">No runtime connected. Terminal execution is not started.</p><div className="mt-4 flex items-center gap-2"><span className="font-mono text-cos-accent">›</span><input ref={inputRef} value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") setInput(""); }} placeholder="Connect a runtime to enter a directive" className="flex-1 bg-transparent font-mono text-sm text-cos-text outline-none placeholder:text-cos-steel/70" /></div></div>
      </div>
    </motion.div>}
    </AnimatePresence>
  );
