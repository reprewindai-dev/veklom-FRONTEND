...
export function MeasureHarness() {
 ...
 if (!next) {
 if (result.record.status === 404) {
 setError(`No VNP measurement exists for execution ${requestedExecutionId} (404). Absent probes are absent proof, not a pending state.`);
 } else {
 setError(result.record.error ?? 'Measurement unavailable');
 }
 return;
 }
