...
export const VnpTelemetryView: React.FC = () => {
 ...
 className="px-3.5 py-1.5 rounded-xl bg-theme-surface border border-theme-border text-slate-300 font-mono text-xs hover:text-white flex items-center gap-2 cursor-pointer"
 >
 <RefreshCw className={`w-3.5 h-3.5 ${isLoading ? 'animate-spin text-theme-accent' : ''}`} /> Refresh Telemetry
 </button>
 </div>
 ...
 <span className="text-slate-400">Next step: <strong className="text-theme-accent">Manual step</strong></span>
 </div>
 </div>
 ...
 <thead>
 <tr className="text-2xs text-slate-400 border-b border-theme-border">
 ...
 {leases.map((l) => (
 <tr key={l.leaseId} className="hover:bg-theme-surface">
 <td className="py-3 font-bold text-theme-accent font-mono">
 {l.token}
