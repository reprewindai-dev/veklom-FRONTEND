...
export default function AmbientIntervention() {
 ...
 eventData?.type ==="PAYMENT_REQUIRED" ?"bg-emerald-500/20" :"bg-theme-accent/20"
 }`} />
 ...
 eventData?.type ==="PAYMENT_REQUIRED" ?"border-emerald-500/30 bg-emerald-500/10 text-emerald-400" :"border-theme-border bg-theme-accent/10 text-theme-accent"
 }`}>
 ...
 eventData?.type ==="PAYMENT_REQUIRED" ?"VNP Micro-Stake Required" :"Ambient Intervention"}
 </h2>
 <p className={`text-sm ${
 eventData?.type ==="QUARANTINE" ?"text-amber-300" : 
 eventData?.type ==="PAYMENT_REQUIRED" ?"text-emerald-300" :"text-theme-accent/70"
 }`}>
 ...
 <KeyRound size={16} className="text-theme-accent" />
 Provider API Key
 </label>
 ...
 className="w-full rounded-xl border border-white/10 bg-black/50 px-4 py-3 text-white focus:border-theme-border focus:outline-none"
 autoFocus
 />
 </div>
 </div>
 <div className="flex justify-end gap-3 pt-4">
 <button type="submit" className="rounded-xl bg-theme-accent px-6 py-2.5 text-sm font-medium text-white hover:bg-theme-accent">
