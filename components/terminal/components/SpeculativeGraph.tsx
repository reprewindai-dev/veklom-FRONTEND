...
export const SpeculativeGraph: React.FC<SpeculativeGraphProps> = ({ paths }) => {
 ...
 <span className="text-[10px] font-mono text-theme-accent bg-cyan-400/10 px-2 py-0.5 rounded border border-theme-border">Active: {paths.filter(p => p.status === 'active').length}</span>
 ...
 <div className={`p-1.5 rounded-lg ${path.status === 'active' ? 'bg-theme-accent/20 text-theme-accent' : 'bg-red-500/10 text-red-400'}`}>
 ...
 <p className="text-[12px] leading-relaxed mb-3 opacity-80 italic">"{path.insight}"
 </p>
