...
export const UACPLayers: React.FC<UACPLayersProps> = ({ layers }) => {
 ...
 <span className="text-[9px] font-mono text-theme-accent/50 uppercase">Federated Control Plane</span>
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
 {layers.map((l) => (
 <div key={l.layer} className="p-4 rounded-xl bg-black/40 border border-white/5 flex flex-col gap-3 group hover:border-theme-border transition-all relative overflow-hidden">
 <div className="flex items-center justify-between">
 <div className="p-1.5 rounded bg-cyan-400/10 text-theme-accent">
 {layerIcons[l.layer]}
 </div>
 ...
 <span className="text-[10px] font-mono text-theme-accent/70">{l.latency}ms</span>
 </div>
 ...
 className="absolute bottom-0 left-0 h-[1px] w-full bg-theme-surface from-transparent to-transparent"
 />
 )}
 </div>
 ))}
