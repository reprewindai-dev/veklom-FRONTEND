...
export const IdentityGovernancePanel: React.FC<IdentityGovernancePanelProps> = ({ data }) => {
 ...
 <UserCheck size={18} className="text-theme-accent" />
 <h2 className="text-sm font-mono uppercase tracking-widest text-white/80">Identity Governance (XAA)</h2>
 </div>
 <span className="text-[9px] font-mono text-theme-accent/50 uppercase italic">Beyond the User-Extension Model</span>
 </div>
 ...
 <div className="col-span-2 p-3 rounded-xl bg-cyan-400/5 border border-theme-border flex items-center gap-3">
 <div className={`w-2 h-2 rounded-full ${data.secretless_mode ? 'bg-cyan-400 shadow-[0_0_8px_rgba(34,211,238,0.5)]' : 'bg-white/20'}`} />
 <span className="text-[10px] font-mono uppercase text-theme-accent font-bold">Secretless Auth (OIDC) Optimized</span>
 </div>
 </div>
 </div>
