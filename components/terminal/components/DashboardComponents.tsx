...
export const MetricCard = ({ label, value, subtext, empty = false }: { label: string, value: string, subtext?: string, empty?: boolean }) => {
 ...
 <div className="absolute top-0 left-0 w-full h-[1px] bg-theme-surface from-transparent via-[#00E5FF]/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
...
export const TimelineEvent = ({ title, time, detail, isAlert = false }: { title: string, time: string, detail: string, isAlert?: boolean }) => {
 ...
 <div className={`w-1.5 h-1.5 ${dotColor} rounded-full group-hover:animate-ping`}></div>
 </div>
 <div className="flex justify-between items-baseline mb-1">
 <div className={`${titleColor} text-[11px] font-sans tracking-wide font-medium`}>{title}</div>
...
export const LaunchpadCard = ({ title, status, count, urgency }: { title: string, status: string, count: number, urgency: 'high' | 'normal' | 'low' }) => {
 ...
 return (
 <div className={`obsidian-glass border border-[#00E5FF]/10 rounded-xl p-3.5 cursor-pointer group flex flex-col gap-3 relative overflow-hidden ${urgencyGlows[urgency]}`}>
 <div className="absolute top-0 inset-x-0 h-[1px] bg-theme-surface from-transparent via-[#00E5FF]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
