...
export const GnomledgerExplorerView: React.FC = () => {
 ...
 className="w-full bg-theme-surface border border-theme-border rounded-xl p-3 font-mono text-2xs text-slate-200 focus:outline-none focus:border-emerald-500"
 ></textarea>
 ...
 {verificationOutput && (
 <pre className="p-3 bg-theme-surface rounded-xl border border-emerald-500/40 text-emerald-300 font-mono text-3xs whitespace-pre-wrap">
 {verificationOutput}
 </pre>
 )}
 </div>
 ...
 <div key={block.certId} className="p-4 bg-theme-surface rounded-xl border border-theme-border space-y-2 text-2xs">
 <div className="flex items-center justify-between text-emerald-400 font-bold border-b border-theme-border pb-2">
 ...
 <div><span className="text-slate-400">Execution Identity:</span> <span className="text-theme-accent">{block.executionIdentityToken}</span></div>
 ...
 <div><span className="text-slate-400">Ed25519 Signature:</span> <span className="text-theme-accent truncate block">{block.verifierSignature}</span></div>
 </div>
 </div>
 ))}
 </div>
