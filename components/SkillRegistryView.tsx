    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8 text-slate-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-slate-800">
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-xs tracking-wider uppercase shadow-md flex items-center gap-2 shrink-0 cursor-pointer"
        >
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-900 p-4 rounded-xl border border-slate-800">
        <div className="relative w-full sm:w-96">
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-lg pl-9 pr-4 py-2 text-xs font-mono text-slate-200 focus:outline-none focus:border-cyan-500"
          />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredSkills.map((s) => (
          <div key={s.id} className="bg-slate-900 p-5 rounded-2xl border border-slate-800 space-y-4 hover:border-slate-700 transition-all flex flex-col justify-between">
            <div className="space-y-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {(s.eucCompatible || s.eccCompatible) && (
                      <span className="px-2 py-0.5 rounded bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 text-3xs font-mono font-bold">
                        EUC
                      </span>
                    )}
                  </div>
              ...
              <div className="p-2.5 bg-slate-950 rounded-lg border border-slate-800/80 space-y-1 font-mono text-3xs">
                <div><span className="text-slate-400">Author:</span> <span className="text-slate-200">{s.author}</span></div>
                <div><span className="text-slate-400">SHA-256 Hash:</span> <span className="text-cyan-400 truncate block">{s.hash.substring(0, 28)}...</span></div>
              ...
              {s.codeSnippet && (
                <div className="space-y-1">
                  ...
                  <pre className="p-2.5 bg-slate-950 rounded-lg border border-slate-800 text-3xs font-mono text-slate-300 overflow-x-auto max-h-24">
                    {s.codeSnippet}
                  </pre>
                </div>
              )}
            ...
            <div className="pt-3 border-t border-slate-800 flex items-center justify-between text-3xs font-mono">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-2xl w-full p-6 space-y-5 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="font-bold text-base text-white flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-cyan-400" /> RepoGate Skill Intake & Security Scanner
              </h3>
              <button
            ...
            <div className="space-y-3 text-xs font-mono">
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-2xs text-slate-200 focus:outline-none focus:border-cyan-500"
                ></textarea>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800">
              ...
              <button
                ...
                className="px-5 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-mono font-bold text-xs uppercase flex items-center gap-2 cursor-pointer disabled:opacity-50"
              >
                {isScanning ? (
                  <>
