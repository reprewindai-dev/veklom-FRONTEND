...
export function WalletBlock({ wallet, onConnect, onDisconnect }: WalletBlockProps) {
 ...
 return (
 <div id="wallet-terminal" className="bg-[#0d0f16] border border-white/10 rounded-lg p-5 relative overflow-hidden shadow-lg shadow-theme-accent/20">
 {/* Background cyber accent */}
 <div className="absolute top-0 right-0 w-24 h-24 bg-theme-accent/5 blur-2xl pointer-events-none" />
 ...
 <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-theme-accent/10 text-theme-accent border border-theme-border">
 {wallet.network}
 </span>
 </div>
 ...
 Link your signature wallet via <span className="text-theme-accent font-semibold">Web3 Connection</span> or the <span className="text-theme-accent font-semibold">Veklom Registry</span> for ERC-404 / x402 consensus game tracking.
 </p>
 ...
 className="w-full bg-theme-surface text-white hover: hover: disabled:opacity-50 text-xs font-bold uppercase tracking-wider py-2.5 px-4 rounded transition-all duration-150 flex items-center justify-center gap-2 shadow shadow-theme-accent/20 font-sans"
 >
 {isVerifying ? (
 <>
 ...
 readOnly
 placeholder="0x..."
 className="flex-1 bg-[#050608] border border-white/10 rounded px-3 py-2 text-xs font-mono text-white focus:outline-none focus:border-theme-border"
 />
 <button
 id="btn-verify-wallet"
 disabled
 className="bg-theme-surface text-slate-500 disabled:opacity-50 text-xs font-bold uppercase tracking-wider px-4 py-2 rounded transition-all duration-150 flex items-center gap-1.5 font-sans border border-white/5 cursor-not-allowed"
 >
 Needs BYOS Auth
 </button>
 </div>
