...
export function ChallengesBlock({ challenges, onClaimReward, onClaimAll, bankroll }: ChallengesBlockProps) {
 ...
 case 'bot_cycles':
 return <Cpu className="w-5 h-5 text-theme-accent animate-pulse" />;
 case 'high_roller':
 return <Sparkles className="w-5 h-5 text-pink-500" />;
 default:
 return <Award className="w-5 h-5 text-theme-accent" />;
 }
 };
 ...
 <div className="w-12 h-12 rounded-full bg-theme-accent/10 border border-theme-border flex items-center justify-center text-theme-accent">
 ...
 <div className="text-lg font-black font-mono text-theme-accent">{unclaimedCount} pending</div>
 </div>
 </div>
 <button
 disabled={unclaimedCount === 0}
 onClick={onClaimAll}
 className="w-full mt-2 py-1.5 bg-theme-surface from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 active:scale-95 disabled:opacity-30 disabled:scale-100 transition-all text-black font-mono text-[10px] uppercase font-black rounded-md tracking-wider flex items-center justify-center gap-1.5"
 >
 ...
 <div className="bg-[#0b0c10] border border-white/10 rounded-lg p-5 space-y-4 shadow-lg shadow-theme-accent/20">
 ...
 <span className="text-[9px] font-mono bg-theme-accent/10 text-theme-accent tracking-wider py-1 px-2.5 rounded border border-theme-border uppercase">
 ...
 {challenge.claimed ? (
 <span className="text-[8px] font-mono font-bold uppercase tracking-wider bg-theme-surface text-slate-400 px-1.5 py-0.5 rounded border border-white/5">
 ✓ Claimed
 </span>
 ) : challenge.completed ? (
 ...
 <span className="text-[8px] font-mono font-bold uppercase tracking-wider bg-theme-accent/10 text-theme-accent px-1.5 py-0.5 rounded border border-theme-border">
 In Progress
 </span>
 )}
 </div>
 ...
 ? 'bg-theme-surface'
 : challenge.completed
 ? 'bg-theme-surface from-[#10b981] to-[#059669]'
 : 'bg-theme-surface '
 }`}
 ...
 <button 
 disabled
 className="py-1 px-3 border border-white/5 bg-theme-surface rounded font-mono text-[9px] uppercase font-bold text-slate-500 cursor-not-allowed flex items-center gap-1"
 >
 ...
 className="py-1.5 px-4.5 bg-theme-surface from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 text-black rounded font-mono text-[10px] uppercase font-black tracking-wider transition-all hover:scale-105 active:scale-95 cursor-pointer shadow-md shadow-emerald-500/10 flex items-center gap-1 border border-emerald-400/20"
 >
