...
export const FlameMeter: React.FC<FlameMeterProps> = ({ winStreak }) => {
 ...
 if (winStreak === 0) return { title: 'Cold Engine', desc: 'No active streak. Initialize arena route.', color: 'text-slate-500', barColor: 'bg-theme-surface', bgGlow: ' to-transparent', label: 'STABLE' };
 ...
 if (winStreak === 4) return { title: 'Nuclear fusion', desc: 'Base gas optimizing. Viral multipliers expected.', color: 'text-pink-500', barColor: 'bg-theme-surface from-red-500 to-pink-500 shadow-pink-500/50', bgGlow: 'from-pink-500/20 to-transparent', label: 'OVERDRIVE' };
 if (winStreak >= 5 && winStreak < 8) return { title: 'Cosmic Inferno', desc: 'Total protocol domination. Unstoppable state.', color: 'text-theme-accent', barColor: 'bg-theme-surface from-pink-500 shadow-theme-accent/20', bgGlow: ' to-transparent', label: 'UNSTOPPABLE' };
 return { title: 'Singularity Breach', desc: 'x402 hyper-consensus reached. Base.org Protocol Legend.', color: 'text-theme-accent animate-pulse', barColor: 'bg-theme-surface via-pink-500 shadow-theme-accent/20', bgGlow: ' to-transparent', label: 'GOD-MODE' };
 }, [winStreak]);
 ...
 : 'border-white/10 shadow-theme-accent/20'
 }`}
 >
 {/* Dynamic Glowing Background Accent */}
 <div className={`absolute inset-0 bg-theme-surface ${streakLevel.bgGlow} pointer-events-none transition-all duration-500`} />
 ...
 <div
 key={ember.id}
 className={`absolute bottom-0 rounded-full bg-theme-surface ${
 winStreak >= 5 ? ' ' : winStreak >= 3 ? 'from-red-500 to-orange-400' : 'from-orange-500 to-yellow-300'
 } opacity-70`}
 style={{
 left: ember.left,
 width: `${ember.size}px`,
 ...
 : 'bg-theme-surface border-white/5'
 }`}>
 {winStreak >= 5 ? (
 ...
 winStreak >= 5 
 ? 'bg-theme-accent/20 text-theme-accent/70 border border-theme-border animate-pulse' 
 : winStreak >= 3 
 ? 'bg-red-500/20 text-red-300 border border-red-500/30' 
 : winStreak > 0 
 ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20' 
 : 'bg-theme-surface text-slate-500 border border-white/5'
 }`}>
 {streakLevel.label}
 </span>
 </div>
