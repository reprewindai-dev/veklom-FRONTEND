...
export default function LedgerExplorer({ ledger }: LedgerExplorerProps) {
 ...
 complianceStandards: ["GDPR Article 32 attestation logging","SOC 2 Trust Services Criteria (CC6.1, CC6.3)","ISO/IEC 27001 secure audit logging standards"
 ],
 metadata: {
 ...
 className="w-full bg-[#0B0C0E] border border-[#23272E] pl-9 pr-4 py-2 text-xs rounded text-[#D1D5DB] focus:outline-none focus:border-theme-border font-mono"
 />
 </div>
 <div className="flex flex-wrap items-center gap-2 select-none w-full sm:w-auto justify-end">
 <button
 onClick={handleExportAuditTrail}
 className="text-[10px] text-theme-accent bg-[#1A1D23] hover:bg-[#20242D] hover:text-[#FFFFFF] border border-[#23272E] hover:border-theme-border px-3 py-1.5 rounded flex items-center gap-1.5 font-mono font-bold transition-all uppercase tracking-wider"
 title="Generate custom compliance-ready formatted JSON report"
 >
 <FileJson className="w-3.5 h-3.5 text-theme-accent" />
 Export Audit Trail
 </button>
 ...
 className="text-[10px] text-theme-accent hover:underline flex items-center gap-1 font-mono hover:text-theme-accent/70 pointer-events-auto h-4 text-left uppercase"
 >
 ...
 <div className="text-gray-500 font-bold">âž” HASH SNAPSHOT DETAILS:</div>
 ...
 <span className="text-theme-accent break-all bg-[#0B0C0E] p-1 rounded inline-block w-full border border-[#23272E]">{ev.why.policyApplied} ({ev.why.policyVersion})</span>
 </div>
 </div>
 ...
 <div>Capability terminal output: <code className="text-[10px] text-theme-accent font-mono bg-[#0B0C0E] px-1 py-0.5 border border-[#23272E] rounded break-all select-text block mt-1">{ev.result.outputSummary}</code></div>
 </div>
 </div>
 )}
 </div>
 ...
 className="w-full md:w-32 py-1 text-[10px] font-mono bg-[#1A1D23] text-theme-info border border-[#23272E] text-theme-accent hover:text-white hover:bg-[#1E222B] font-bold rounded tracking-wider uppercase transition-all"
 >
 Export block
 </button>
 </div>
 ...
 <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
 <FileJson className="w-4 h-4 text-theme-accent" />
 Evidence Export Hub
 </h3>
 ...
 ðŸ’¡ Drag / Highlight raw text to export snapshot manually.
 </div>
 </div>
 ) : (
