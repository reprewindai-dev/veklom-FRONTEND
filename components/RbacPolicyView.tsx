...
export const RbacPolicyView: React.FC<RbacPolicyViewProps> = ({ currentRole, setRole }) => {
 ...
 <div className="flex items-center gap-2 bg-theme-surface p-2 rounded-xl border border-theme-border text-xs font-mono">
 <span className="text-slate-400">Active Identity Role:</span>
 <select
 value={currentRole}
 onChange={(e) => setRole(e.target.value as UserRole)}
 className="bg-theme-surface text-theme-accent font-bold border border-theme-border px-3 py-1 rounded focus:outline-none"
 >
 ...
 <div className="bg-theme-surface rounded-2xl border border-theme-border overflow-hidden font-mono text-xs">
 <table className="w-full text-left border-collapse">
 <thead>
 <tr className="bg-theme-surface text-slate-400 uppercase text-3xs border-b border-theme-border">
 ...
 return (
 <tr key={r} className={isCurrent ? 'bg-theme-accent/10' : 'hover:bg-theme-surface'}>
 <td className="p-4 font-bold text-white flex items-center gap-2">
 {r.toUpperCase()}
 {isCurrent && (
 <span className="px-2 py-0.5 rounded bg-theme-accent/20 text-theme-accent text-3xs font-bold border border-theme-border">
 ACTIVE
 </span>
 )}
 </td>
