...
export default function AnomalyDetector({ onAppendLedger, onStateUpdate, onTriggerRealtimeNotification }: AnomalyDetectorProps) {
 ...
 const [injectedNoise, setInjectedNoise] = useState<Record<string, number>>({"RES-001": 0,"RES-002": 0,"RES-003": 0,"RES-004": 0,
 });
 ...
 <Activity className="text-theme-accent w-5 h-5 animate-pulse" />
 ...
 activeResId === res.id 
 ? 'bg-theme-surface2 border-theme-border shadow-[0_0_12px_rgba(6,182,212,0.15)]' 
 : 'bg-[#05070a]/40 border-theme-border hover:border-theme-border'
 }`}
 >
 ...
 <div className="flex items-center justify-between text-xs font-mono text-slate-400 mb-3 bg-[#0d1x17]/40 p-2 rounded border border-theme-border">
 <div>
 <span>CURR: </span>
 <span className={`font-bold ${
 res.state === 'anomaly' ? 'text-red-500 font-extrabold' : res.state === 'warning' ? 'text-amber-400' : 'text-theme-accent'
 }`}>
 {res.value} {res.unit}
 </span>
 </div>
 ...
 className="w-full h-1.5 bg-theme-surface rounded-lg appearance-none cursor-pointer accent-cyan-500"
 />
 ...
 <div className="mt-4 pt-4 border-t border-theme-border flex justify-between items-center text-xs font-mono">
 ...
 alpha === val 
 ? 'bg-theme-surface2 border-theme-border text-theme-accent/70 shadow-[0_0_8px_rgba(6,182,212,0.2)] font-semibold'
 : 'bg-[#0d1117] border-theme-border text-slate-500'
 }`}
 >
 {val} ({val === 0.05 ?"95%" :"99%"})
 </button>
 ...
 <span className="text-xs font-mono text-theme-accent uppercase tracking-widest font-semibold">
 ...
 <div className="w-full h-[200px] border border-theme-border bg-[#05070a]/90 rounded-lg p-2 relative">
 ...
 <span className={`font-bold ${fStat > fCritical ? 'text-red-400' : 'text-theme-accent'}`}>
 {fStat}
 </span>
 </div>
 <div className="flex justify-between items-center py-1 border-b border-theme-border">
 ...
 <span className={`font-bold ${pValue < alpha ? 'text-red-400' : 'text-theme-accent'}`}>
 {pValue}
 </span>
 </div>
 </div>
 ...
 {/* Filtering Interface */}
 <div className="mb-4 bg-[#05070a]/60 border border-theme-border rounded-lg p-2.5 flex flex-col gap-2">
 <div className="flex items-center justify-between">
 <span className="text-[9px] uppercase font-mono text-theme-accent font-extrabold tracking-wider">
 ...
 fFilter === 'all'
 ? 'bg-theme-surface2 border-theme-border text-theme-accent font-bold shadow-[0_0_8px_rgba(6,182,212,0.1)]'
 : 'bg-transparent border-theme-border text-slate-400 hover:text-slate-200 hover:border-theme-border'
 }`}
 ...
 fFilter === 'custom'
 ? 'bg-theme-surface2 border-theme-border text-theme-accent/70 font-bold shadow-[0_0_8px_rgba(6,182,212,0.1)]'
 : 'bg-transparent border-theme-border text-slate-500 hover:text-slate-300 hover:border-theme-border'
 }`}
 >
 Custom Range Slider...
 </button>
 ...
 {fFilter === 'custom' && (
 <div className="space-y-1.5 mt-1 pt-2 border-t border-theme-border text-[9px] font-mono text-slate-400">
 <div className="flex justify-between items-center">
 <span>Min F-Ratio:</span>
 <span className="text-theme-accent font-bold">{customRangeMin.toFixed(1)}</span>
 </div>
 <input
 type="range"
 min="1.0"
 ...
 className="w-full h-1 bg-theme-surface rounded appearance-none cursor-pointer accent-cyan-500"
 />
 ...
 <span className="text-theme-accent font-bold bg-theme-surface2 border border-theme-border px-1.5 py-0.5 rounded">F: {anom.testStatF.toFixed(2)}</span>
 </div>
 </div>
 ))
 )}
 ...
 <span className="text-theme-accent font-bold">L5 Active Enforce</span>
 </div>
 </div>
 </div>
