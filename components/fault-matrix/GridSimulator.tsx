...
export default function GridSimulator({ onAppendLedger, onStateUpdate }: GridSimulatorProps) {
 ...
 const names = ["Alpha-Plan","Beta-Scribe","Delta-Sync","Sigma-Crypt","Omega-Sec","Gamma-Route","Tau-Audit","Zeta-Filter","Kappa-Guard","Rho-Ledger"
 ];
 for (let i = 1; i <= 100; i++) {
 ...
 let classes ="w-full aspect-square border border-theme-border flex flex-col justify-between p-1 transition-all duration-205 relative select-none";
 ...
 if (cell.type === 'start') {
 classes +="bg-theme-surface2 text-theme-accent font-bold border border-theme-border shadow-[0_0_12px_rgba(6,182,212,0.4)]";
 } else if (cell.type === 'target') {
 classes +="bg-amber-950/65 text-amber-400 font-bold border border-amber-500 shadow-[0_0_12px_rgba(245,158,11,0.4)]";
 } else if (cell.type === 'obstacle') {
 classes +="bg-[#111622] border border-theme-border shadow-inner";
 } else if (isCurrent) {
 classes +="bg-theme-info text-slate-950 font-black border border-theme-border animate-pulse shadow-[0_0_15px_rgba(34,211,238,0.6)]";
 } else if (isTraversed) {
 classes +="bg-theme-surface2 border border-theme-border text-theme-accent/70/80";
 } else {
 classes +="bg-[#05070a]/40 text-slate-700 hover:bg-theme-surface2 cursor-pointer";
 }
 ...
 <Layers className="text-theme-accent w-5 h-5 animate-pulse" />
 ...
 ? 'bg-theme-surface2 border-theme-border text-theme-accent/70 font-semibold shadow-[0_0_8px_rgba(6,182,212,0.25)]'
 : 'bg-[#111622] border-theme-border text-slate-500 hover:text-slate-400'
 }`}
 >
 ðŸš§ Wall Brush
 </button>
 ...
 className={`p-2 rounded text-xs font-mono border transition-all cursor-pointer ${
 placementMode === 'start'
 ? 'bg-theme-surface2 border-theme-border text-theme-accent/70 font-semibold shadow-[0_0_8px_rgba(6,182,212,0.35)]'
 : 'bg-[#111622] border-theme-border text-slate-500 hover:text-slate-400'
 }`}
 >
 ðŸŸ¢ Set Start
 ...
 className="flex-1 py-3 px-4 bg-theme-accent hover:bg-theme-info disabled:bg-theme-surface disabled:text-slate-650 text-[#05070a] font-mono font-bold rounded-lg text-xs flex items-center justify-center gap-2 transition-all cursor-pointer shadow-[0_0_15px_rgba(6,182,212,0.35)]"
 >
 <Play className="w-3.5 h-3.5 fill-[#05070a]" /> RUN PATHFINDER
 </button>
 <button
 onClick={resetAll}
 className="py-3 px-3 bg-[#0d1117] hover:bg-theme-surface border border-theme-border text-slate-350 font-mono rounded-lg text-xs flex items-center justify-center transition-all cursor-pointer"
 ...
 <div className="border border-theme-border bg-theme-surface2 p-3 rounded-lg shadow-inner">
 <div className="flex justify-between items-center mb-2">
 <span className="text-xs font-mono text-theme-accent tracking-wider font-semibold">CONCURRENT OPERATIONS</span>
 <span className="text-xs font-mono bg-theme-surface2 text-theme-accent/70 px-2 py-0.5 rounded border border-theme-border">120 Workers</span>
 </div>
 ...
 const stateColors = ["bg-theme-info","bg-theme-info","bg-amber-400","bg-theme-info"];
 ...
 <span className="text-theme-accent/80 font-bold">Active Queue Concurrency: {activeAgentCount} Threads</span>
 </div>
 </div>
 </div>
 ...
 <div className="bg-[#0d1117] p-2 rounded border border-theme-border">
 <span className="text-slate-500 block text-[9px]">KV CACHE HITS</span>
 <span className="text-theme-accent font-semibold">{isSimulating ?"98.4%" :"100.0%"}</span>
 </div>
 <div className="bg-[#0d1117] p-2 rounded border border-theme-border">
 <span className="text-slate-500 block text-[9px]">PAGEDATTENTION</span>
 <span className="text-theme-accent font-semibold text-xs">Active (16kb block)</span>
 </div>
 </div>
 </div>
 </div>
 ...
 {/* Pathfinder Grid */}
 <div className="lg:col-span-5 bg-[#0a0c14]/85 border border-theme-border p-4 rounded-xl flex flex-col items-center justify-between shadow-xl">
 <div className="w-full flex justify-between items-center mb-3">
 <span className="text-xs font-mono text-theme-accent font-semibold tracking-wider">VECTOR SPACE DESCENT MATRIX</span>
 ...
 {/* Main 2D Grid */}
 <div className="w-full max-w-[420px] aspect-square grid grid-cols-10 gap-[2px] bg-theme-surface bg-opacity-20 p-2 rounded-lg border border-theme-border shadow-2xl">
 {grid.map((row) =>
 row.map((cell) => {
 ...
 {cell.type === 'obstacle' ? 'â–ˆ' : cellPotential}
 </span>
 </div>
 );
 })
 ...
 {/* Grid Legends */}
 <div className="w-full grid grid-cols-3 gap-2 mt-4 text-[10px] font-mono text-slate-400 border-t border-theme-border pt-3">
 <div className="flex items-center gap-1.5 justify-center">
 <span className="w-2.5 h-2.5 bg-theme-info border border-theme-border rounded-sm inline-block animate-pulse" />
 <span>ðŸŸ¢ Start</span>
 </div>
 <div className="flex items-center gap-1.5 justify-center">
 ...
 <div className="flex items-center gap-1.5 justify-center">
 <span className="w-2.5 h-2.5 bg-theme-surface border border-theme-border rounded-sm inline-block" />
 <span>â–ˆ Obstacles</span>
 </div>
 </div>
 </div>
 ...
 <div className="bg-[#0a0c14]/85 border border-theme-border p-4 rounded-xl flex-1 shadow-xl">
 ...
 : 'bg-[#0d1117] border-theme-border hover:order-slate-800'
 }`}
 >
 ...
 <div className="text-theme-accent/95 mt-1">
 <span className="text-theme-accent uppercase font-extrabold mr-1">RECOVERY:</span> {f.recovery}
 </div>
 </div>
 )}
 </div>
