...
const CustomGpcNode = React.memo<CustomGpcNodeProps>(
 ...
 ${selected ? 'border-theme-border shadow-lg' : 'border-gray-300'}
 ${statusColor}
 ...
 <div className="text-[10px] text-theme-inkDim mt-1">
 Running...
 </div>
 )}
...
const CustomEdge = React.memo<CustomEdgeProps>(
 ...
 <path
 d={d}
 stroke={selected ? 'var(--theme-accent)' : '#d1d5db'}
 strokeWidth={selected ? 2 : 1.5}
 fill="none"
 className="transition-all"
 />
 {selected && (
 <circle cx={targetX} cy={targetY} r={4} fill="var(--theme-accent)" />
 )}
 </g>
 );
 }
...
export const GpcCanvas: React.FC<GpcCanvasProps> = React.memo(
 ...
 className="px-3 py-1 text-sm font-medium rounded bg-theme-accent text-white hover:bg-theme-accent/80 disabled:opacity-50"
 >
 Compile
 </button>
 <button
 ...
 <MiniMap
 nodeColor={(n) =>
 selectedNodeIds.has(n.id) ? 'var(--theme-accent)' : '#10b981'
 }
 />
 </ReactFlow>
 </div>
