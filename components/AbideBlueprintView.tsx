    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8 text-slate-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-slate-800">
        ...
        <div className="flex items-center gap-2 text-2xs font-mono">
          <span className="px-2.5 py-1 rounded bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 font-bold">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 space-y-4">
            <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider font-mono flex items-center gap-2">
              <FileText className="w-4 h-4 text-indigo-400" /> Enter Raw Developer Intent
            </h3>
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-gradient-to-r from-indigo-600 via-cyan-600 to-emerald-600 hover:from-indigo-500 hover:to-emerald-500 text-white font-bold text-xs tracking-wider uppercase shadow-lg shadow-indigo-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
            >
              {isCompiling ? (
                <>
              ...
              <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 space-y-4 font-mono text-xs">
                <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                  <div className="text-slate-300 font-bold">
                    ABIDE BLUEPRINT: <span className="text-indigo-400">{blueprint.blueprintId}</span>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center text-2xs">
                  ...
                  <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
                    ...
                    <div className="text-cyan-400 font-bold text-3xs mt-1 truncate">
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <div className="space-y-3">
                    {blueprint.compiledSteps.map((step, idx) => (
                      <div key={step.stepId} className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-2">
                        <div className="flex items-center justify-between text-xs font-bold text-indigo-300">
                          <span className="flex items-center gap-2">
                            <span className="w-5 h-5 rounded-full bg-indigo-500/20 text-indigo-400 flex items-center justify-center text-3xs">
                              {idx + 1}
                            </span>
                            {step.title}
                          </span>
                        ...
                        <div className="flex items-center gap-4 text-3xs text-slate-400 font-mono">
                          ...
                          <span>Harness Rec: <strong className="text-cyan-400 uppercase">{step.harnessRecommendation}</strong></span>
                        </div>

                        <ul className="text-3xs text-slate-400 space-y-1 pt-1 border-t border-slate-800/80 pl-2">
