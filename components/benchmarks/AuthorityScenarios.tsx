...
const SCENARIOS: AuthorityScenario[] = [
 ...
 icon: Coins,
 colorClass:"text-theme-accent bg-theme-accent/5",
...
export default function AuthorityScenarios() {
 ...
 <span className="text-[9px] bg-neutral-950 px-2 py-0.5 rounded text-theme-accent border border-neutral-800 font-bold uppercase font-mono tracking-wider animate-pulse">
 ...
 <Clock className="w-3.5 h-3.5 text-theme-accent" /> Ground Law Zero
 </div>
 ...
 <SelectedIcon className="w-5 h-5 text-theme-accent" />
 </div>
 ...
 <Terminal className="w-3.5 h-3.5 text-theme-accent" /> TTY::ENCLAVE-SESSION::VK-{currentScenario.id.toUpperCase()}
 </span>
 <span className="text-[8px] bg-neutral-900 border border-neutral-850/70 px-2 py-0.5 text-theme-accent select-none uppercase font-bold">
 ...
 const statusColor = 
 step.status ==="success" ?"text-theme-accent bg-theme-surface2 border-theme-border" :
 step.status ==="warning" ?"text-amber-400 bg-amber-900/10 border-amber-500/20 animate-pulse" :
 step.status ==="error" ?"text-rose-500 bg-rose-500/10 border-rose-500/30 font-black animate-pulse" :"text-neutral-300 bg-neutral-950 border-neutral-850";
 ...
 :"bg-theme-accent hover:bg-cyan-400 text-black shadow-md shadow-theme-accent/20"
 }`}
 >
