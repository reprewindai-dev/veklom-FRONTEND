...
const AVATAR_BASES: AvatarBase[] = [
 { id:"sentinel", name:"Sentinel", avatar:"ðŸ›¡ï¸", description:"Defensive & security orchestration layer.", icon: Shield },
 { id:"oracle", name:"Oracle", avatar:"ðŸ§ ", description:"Deep analysis & cognitive intelligence node.", icon: Brain },
 { id:"forge", name:"Forge", avatar:"ðŸ› ï¸", description:"Synthesizer of structures and software code.", icon: Hammer },
 { id:"muse", name:"Muse", avatar:"ðŸŽ¨", description:"Creative copy sculpting & brand narrative strategist.", icon: Palette },
 { id:"swift", name:"Swift", avatar:"âš¡", description:"High-frequency automation & tactical stream handler.", icon: Zap },
];

const PALETTES: PaletteOption[] = [
 { id:"teal", name:"Teal / Black (Governed)", glowColor:"shadow-[0_0_25px_rgba(6,182,212,0.15)]", borderColor:"border-theme-border", textColor:"text-theme-accent", bgColor:"bg-[#080c0d]/80", badgeBg:"bg-theme-accent/10" },
 ...
 { id:"violet", name:"Violet / Navy (Research)", glowColor:"shadow-[0_0_25px_rgba(139,92,246,0.15)]", borderColor:"border-theme-border", textColor:"text-theme-accent", bgColor:"bg-[#0a0912]/80", badgeBg:"bg-theme-accent/10" },
...
export default function CharacterCreator({ onAgentCreated }: CharacterCreatorProps) {
 ...
 return { level:"COMPLIANT SOVEREIGN SECURE", color:"text-theme-accent bg-theme-accent/10 border-theme-border" };
 }
 if (security >= 4) {
 ...
 [ðŸ”®] GOVERNED AGENT IDENTITY BUILDER
 </h3>
 <button 
 ...
 >
 <Dices className="w-3.5 h-3.5 text-theme-accent" /> Auto-Calibrate Profile
 </button>
 </div>
 ...
 ?"bg-theme-accent/10 text-theme-accent border border-theme-border"
 ...
 <div className={`absolute inset-0 rounded-full blur-xl opacity-30 ${selectedPalette ==="teal" ?"bg-theme-accent" : selectedPalette ==="red" ?"bg-rose-500" : selectedPalette ==="violet" ?"bg-theme-accent" : selectedPalette ==="gold" ?"bg-amber-500" :"bg-emerald-500"}`}></div>
 ...
 <span className="text-theme-accent font-black tracking-widest text-[9px] uppercase">PGL PRE-EXECUTION CERTIFICATE</span>
 <span className="text-[7px] text-neutral-600">v1.2</span>
 </div>
 <div><span className="text-neutral-500">status:</span> <span className="text-theme-accent font-bold">committed</span></div>
 ...
 <div className="pt-0.5"><span className="text-neutral-500">genome_hash:</span> <span className="text-theme-accent block break-all font-mono">sha256:{getPglHexStr(selectedBase +"_" + attributes.intelligence +"_" + attributes.speed, 32)}...</span></div>
 <div><span className="text-neutral-500">constitution_hash:</span> <span className="text-theme-accent block break-all font-mono">sha256:{getPglHexStr((characterName ||"unit") +"_" + attributes.security +"_" + attributes.reliability +"_" + selectedPalette, 32)}...</span></div>
 ...
 <div><span className="text-neutral-500">persisted:</span> <span className="text-theme-accent font-bold">true</span></div>
 ...
 <span key={sc} className="text-[7px] px-1 py-0.5 bg-theme-surface2 border border-theme-border rounded text-theme-accent uppercase font-mono">{sc}</span>
 ))
 )}
 </div>
 </div>
 ...
 <span className="text-theme-accent font-black tracking-widest text-[8.5px] uppercase">EXECUTION IDENTITY V1</span>
 <span className="text-[6.5px] text-neutral-600">LAW 0 OBJECT</span>
 </div>
 <div><span className="text-neutral-550 mr-1">execution_id:</span><span className="text-theme-accent">exec_vk_{getPglHexStr(agentId, 12)}</span></div>
 ...
 <div><span className="text-neutral-550 mr-1">genome_hash:</span><span className="text-theme-accent/85 font-mono">sha256:{getPglHexStr(selectedBase +"_" + attributes.intelligence +"_" + attributes.speed, 24)}...</span></div>
 <div><span className="text-neutral-550 mr-1">constitution_hash:</span><span className="text-theme-accent/85 font-mono">sha256:{getPglHexStr((characterName ||"unit") +"_" + attributes.security +"_" + attributes.reliability +"_" + selectedPalette, 24)}...</span></div>
 ...
 <div><span className="text-neutral-550 mr-1">budget_approved_cents:</span><span className="text-amber-400 font-bold">{attributes.speed * 100}Â¢</span></div>
 <div><span className="text-neutral-550 mr-1">delegation_depth:</span><span className="text-theme-accent font-bold">{attributes.creativity}</span></div>
 <div><span className="text-neutral-550 mr-1">ttl_seconds:</span><span className="text-theme-accent font-mono">{attributes.intelligence * 360}s</span></div>
 ...
 <div><span className="text-neutral-550 mr-1">issuer:</span><span className="text-theme-accent">Veklom Authority v5</span></div>
 ...
 className="w-full bg-black/60 p-3 border border-neutral-850 rounded text-neutral-200 placeholder-neutral-600 text-xs focus:outline-none focus:border-theme-border"
 />
 </div>
 ...
 selectedBase === b.id 
 ?"bg-theme-accent/[0.04] border-theme-border text-theme-accent" 
 ...
 p.id ==="teal" ?"bg-theme-info" :
 p.id ==="red" ?"bg-rose-500" :
 p.id ==="violet" ?"bg-theme-accent" :
 ...
 BALANCE: <b className={`font-mono font-black ${pointsRemaining === 0 ?"text-theme-accent" : pointsRemaining < 0 ?"text-rose-500" :"text-amber-400"}`}>{pointsRemaining} NODES</b> REMAINING
 </span>
 <button 
 ...
 <div key={i} className="h-full bg-theme-accent/80 rounded-sm" style={{ width: `${100 / 10}%` }}></div>
 ))}
 </div>
 <span className="w-4 font-bold text-right text-theme-accent">{attributes.intelligence}</span>
 <button 
 onClick={() => handleSetStat("intelligence", attributes.intelligence + 1)}
 ...
 <span className="w-4 font-bold text-right text-theme-accent">{attributes.creativity}</span>
 <button 
 onClick={() => handleSetStat("creativity", attributes.creativity + 1)}
 className="w-5 h-5 bg-neutral-900 border border-neutral-800 text-center text-xs text-neutral-400 rounded hover:text-white hover:border-neutral-700 font-bold active:scale-95"
 >+</button>
 ...
 className="w-full bg-neutral-950 p-2.5 border border-neutral-850 rounded text-neutral-200 placeholder-neutral-700 text-[10.5px] focus:outline-none focus:border-theme-border font-mono"
 />
 ...
 {["Scan files to audit system telemetry buffers","Verify billing records & check database tables","Mutate repo files & run write execution trials","Drop client bills & force outbound exfiltration"
 ].map((txt) => (
 <button
 key={txt}
 type="button"
 ...
 intendedAction === txt
 ?"bg-theme-accent/10 border-theme-border text-theme-accent font-bold"
 ...
 isChecked
 ?"bg-theme-accent/[0.02] border-theme-border text-neutral-200 shadow-sm"
 ...
 isChecked ?"bg-theme-accent/15 border-theme-border text-theme-accent" :"bg-neutral-950 border-neutral-800 text-transparent"
 }`}>
 âœ“
 </div>
 <div className="flex-1 overflow-hidden">
 <span className={`block font-bold truncate ${isChecked ?"text-theme-accent" :"text-neutral-450"}`}>{sc.label}</span>
 ...
 <Lock className="w-3.5 h-3.5 text-theme-accent" /> Veklom Authority Boundary Compatibility Checklist
 </span>
 ...
 <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center font-bold text-[8px] border ${compatibility.pglPass ?"text-theme-accent bg-theme-accent/10 border-theme-border" :"text-neutral-500 border-neutral-800"}`}>
 {compatibility.pglPass ?"âœ“" :"âœ—"}
 </div>
 <div>
 ...
 <div className="flex items-center gap-2.5 bg-neutral-950/40 p-2.5 rounded border border-neutral-850/60">
 <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center font-bold text-[8px] border ${compatibility.sekedPass ?"text-theme-accent bg-theme-accent/10 border-theme-border" :"text-neutral-500 border-neutral-800"}`}>
 {compatibility.sekedPass ?"âœ“" :"âœ—"}
 </div>
 <div>
 ...
 <div className="flex items-center gap-2.5 bg-neutral-950/40 p-2.5 rounded border border-neutral-850/60">
 <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center font-bold text-[8px] border ${compatibility.executionPass ?"text-theme-accent bg-theme-accent/10 border-theme-border" :"text-neutral-500 border-neutral-800"}`}>
 {compatibility.executionPass ?"âœ“" :"âœ—"}
 </div>
 <div>
 ...
 <div className="flex items-center gap-2.5 bg-neutral-950/40 p-2.5 rounded border border-neutral-850/60">
 <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center font-bold text-[8px] border ${compatibility.mcpPass ?"text-theme-accent bg-theme-accent/10 border-theme-border" :"text-neutral-500 border-neutral-800"}`}>
 {compatibility.mcpPass ?"âœ“" :"âœ—"}
 </div>
 <div>
 ...
 <span className={`font-black uppercase text-xs ${pointsRemaining === 0 ?"text-theme-accent" :"text-rose-500"}`}>
 {pointsRemaining === 0 
 ...
 totalPointsSpent === 20
 ?"bg-theme-accent hover:bg-theme-info text-black shadow-[0_0_15px_rgba(6,182,212,0.15)] active:scale-95"
