...
export function getEducationalDetails(matchedRule: string, path: string): EducationalDetails {
 ...
 remediationSteps: ["Revoke and rotate the exposed credentials immediately (treat the old ones as public).","Add '.env', '.key', and '.pem' files to your '.gitignore' file so they are never committed.","Run 'git rm --cached <file_path>' to untrack and remove the files from Git history.","Migrate configurations to Cloud Secret Manager and inject them securely as environment variables."
 ]
 };
 }
 ...
 remediationSteps: ["Audit JWT configurations to ensure they use strong asymmetric algorithms (e.g., RS256) instead of HS256.","Enforce HttpOnly, Secure, and SameSite=Strict on all cookies to prevent cross-site scripting (XSS) leaks.","Configure strict rate-limiting on user login and verification endpoints.","Validate user roles on the server side for every protected API request."
 ]
 };
 }
 ...
 remediationSteps: ["Incorporate official signature verification on webhook receivers using 'stripe.webhooks.constructEvent'.","Securely store your Stripe Webhook Signing Secret in Cloud Secret Manager.","Calculate subscription fees and invoice totals strictly on the backend, never using values from the client.","Conduct quarterly database audits comparing transaction logs against the Stripe ledger."
 ]
 };
 }
 ...
 remediationSteps: ["Never run raw database statements constructed with client inputs (enforce prepared parameterization).","Incorporate PostgreSQL Row-Level Security: 'ALTER TABLE tenant_data ENABLE ROW LEVEL SECURITY;'.","Execute schema migrations strictly using verified automated pipelines, never via active shell prompts.","Set up isolated DB connections with restricted execution permissions for user sessions."
 ]
 };
 }
 ...
 remediationSteps: ["Implement branch protection rules preventing direct master branch pushes to the 'deploy/' directory.","Integrate static manifest checkers (e.g., Kube-Linter or Checkov) inside continuous integration gates.","Set Kubernetes pods to run with non-root privileges and activate read-only root filesystems.","Manage environment secrets securely using specialized cluster controllers or external vault integrations."
 ]
 };
 }
 ...
 remediationSteps: ["Limit GitHub Actions tokens to read-only permissions inside repository settings.","Pin third-party actions to an exact 40-character commit SHA instead of mutable tags.","Require maintainer approval for all workflow executions initiated by external contributors.","Implement branch protection rules requiring approved reviews before pull requests can merge."
 ]
 };
}
...
export const RiskIndicators: React.FC<RiskIndicatorsProps> = ({ 
 ...
 <div className="p-4 flex flex-col justify-between space-y-3 bg-theme-surface from-black to-[#000d02]">
 <div>
 <div className="flex items-center space-x-1">
 ...
 <div className="w-full bg-[#111] h-1.5 rounded-full overflow-hidden">
 <div 
 className="h-full bg-theme-surface from-emerald-500 to-[#00FF41] transition-all duration-500"
 ...
 <div className="p-4 bg-theme-surface from-[#060606] to-[#010a02] border border-[#1b2a1c]/45 rounded-sm flex flex-col md:flex-row gap-4 items-start select-none">
