...
export const ToolExecutor: React.FC = () => {
 ...
 return (
 <div className="flex flex-col gap-4 p-4 h-full bg-[#0b1219]/40 rounded-2xl border border-theme-border overflow-auto scrollbar-hide">
 <div className="flex items-center gap-2 mb-2">
 <Terminal className="w-5 h-5 text-theme-accent" />
 <h2 className="text-sm font-bold tracking-[0.2em] text-theme-accent uppercase">MCP Tool Orchestrator</h2>
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 {/* Tool List */}
 <div className="flex flex-col gap-2">
 <div className="text-[10px] uppercase tracking-widest text-theme-accent/50 mb-1">Available Primitives</div>
 <div className="flex flex-col gap-2">
 {tools.map((tool) => (
 <button
 key={tool.name}
 ...
 ? 'bg-theme-accent/10 border-theme-border shadow-[0_0_15px_rgba(6,182,212,0.1)]'
 : 'bg-[#0f172a]/40 border-theme-border hover:border-theme-border'
 }`}
 >
 <div className="flex items-center justify-between w-full">
 <span className={`text-sm font-medium ${selectedTool?.name === tool.name ? 'text-theme-accent' : 'text-slate-300'}`}>
 {tool.name}
 </span>
 {selectedTool?.name === tool.name && <Zap className="w-3 h-3 text-theme-accent fill-cyan-400" />}
 </div>
 ...
 >
 <div className="bg-[#0f172a]/60 p-4 rounded-xl border border-theme-border backdrop-blur-sm">
 <div className="text-[10px] uppercase tracking-widest text-theme-accent mb-3 flex items-center gap-2">
 ...
 className="text-[9px] text-theme-accent hover:text-theme-accent font-mono uppercase tracking-tighter"
 >
 [ Format ]
 </button>
 </div>
 ...
 isJsonValid 
 ? 'border-theme-border text-theme-accent/70 focus:border-theme-border' 
 ...
 loading
 ? 'bg-theme-surface text-slate-500 cursor-not-allowed'
 : 'bg-theme-accent text-black hover:bg-cyan-400 shadow-[0_0_20px_rgba(6,182,212,0.3)]'
 }`}
 >
 {loading ? (
 ...
 <div className="flex items-center justify-center h-64 border border-dashed border-theme-border rounded-xl bg-theme-surface2">
 <div className="text-[10px] text-theme-inkDim uppercase tracking-widest text-center">
