...
export const LineageLedger: React.FC<LineageLedgerProps> = ({ nodes }) => {
  ...
  return (
    <div className="bg-[#0b1219]/40 rounded-2xl border border-white/10 p-6 flex flex-col gap-4 relative overflow-hidden h-full">
        ...
        {nodes.map((node, i) => (
          ...
          >
            <div className={`w-8 h-8 rounded-full flex items-center justify-center border shadow-lg transition-all ${
              node.type === 'genome' ? 'bg-cyan-500/20 border-cyan-400/50 text-cyan-400' : 'bg-cyan-500/20 border-cyan-400/50 text-cyan-400'
            ...
            <button 
              ...
              className={`flex-1 p-3 rounded-xl bg-black/40 border transition-all flex items-center justify-between group text-left ${
                selectedNode?.id === node.id ? 'border-cyan-400/50 bg-cyan-400/5 shadow-[0_0_15px_rgba(251,191,36,0.1)]' : 'border-white/5 hover:border-white/20'
              }`}
            >
              ...
              <ChevronRight size={14} className={`transition-all ${selectedNode?.id === node.id ? 'text-cyan-400 translate-x-1' : 'text-white/10 group-hover:text-white/40'}`} />
            </button>
          </motion.div>
        ))}
      </div>
        ...
        {selectedNode && (
          ...
          >
            ...
            <div className="flex justify-between items-start mb-4">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-mono uppercase tracking-widest text-cyan-400">Node Proof-of-Trust</span>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="p-2.5 rounded-xl bg-cyan-400/5 border border-cyan-400/20 flex flex-col gap-0.5">
                <div className="flex items-center gap-1.5 text-[9px] font-mono text-cyan-400/50 uppercase tracking-tighter">
                  <Shield size={10} /> Identity Verification
                </div>
                <div className="text-[10px] font-mono text-cyan-400">SIGN_ED25519_PASS</div>
              </div>
            </div>

            <div className="mt-4 flex gap-2">
              <button className="flex-1 py-1.5 rounded-lg bg-cyan-400 text-black text-[10px] font-bold uppercase tracking-wider hover:bg-cyan-300 transition-colors">
                Verify Signature
              </button>
      ...
      <div className="mt-auto p-4 rounded-xl bg-cyan-400/5 border border-cyan-400/10 text-[9px] font-mono leading-relaxed text-white/40 flex flex-col gap-1 backdrop-blur-sm">
         <div className="text-cyan-400 uppercase font-bold text-[10px] flex items-center gap-2">
